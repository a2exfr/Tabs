...
class Tabs {
  ...
  function GetHead() {
	...
	if(  file_exists( $optionsFile ) )	{
		include $optionsFile;
			} else {$options=array(); $options['tabs_state'] = ""; }
	...
	if(  file_exists( $configFile ) )	{
		include $configFile;
			} else {$data = array(); $tab_groups = array();}
	
		
	$tab_group_t = $data;
	foreach ($tab_group_t as $key=>$tab_gp){
		$tab_groups[] = array( 'id'=> $key, 'title' =>$tab_gp['title']);
	}
  ...
  function SectionTypes($section_types) {
    $section_types['Tabs_section'] = array();
  ...
  ';
		...
		if( $opened=="yes"){$opened="in";} else {$opened="";};
	...
	if($appearance == 0 ) { $b ='';} elseif($appearance == 2){$b='class="tabbable tabs-left"';} else {$b='class="tabbable tabs-right"';}
  ...
  function NewSections($links){
	...
	foreach ($links as $key => $section_type_arr) {
		if ( $section_type_arr[0] == 'Tabs_section' ) {
			$links[$key] = array('Tabs_section', $addonRelativeCode . '/img/tabs.jpg');
	  }
  ...
  function PageRunScript($cmd) {
	...
	$sect_options= array('type'=>'Tabs_section','tab_sel'=>$tab_sel,'appearance'=>$appearance,'tabs_id'=>$tabs_id,'panel_class'=>$panel_class, 'opened'=>$opened);
		    
	$arg_value = \gp\tool\Output\Sections::SectionToContent($sect_options,'');
         
    $page->ajaxReplace[] = array('refresh_tabsFn', 'arg', $arg_value);
